...
export default function DashboardPage() {
  ...
  const { business, userId, loading: businessLoading } = useBusiness()
  const { selectedBranchId, branches, canSwitchBranches } = useBranch()

  useEffect(() => {
    if (business && userId) loadData()
    else if (!businessLoading) setLoading(false)
  }, [business, userId, selectedBranchId])
  ...
  const loadData = async () => {
    ...
    let salesQuery = supabase.from('sales').select('*').eq('business_id', business.id).gte('created_at', startOfDay)
    let expensesQuery = supabase.from('expenses').select('*').eq('business_id', business.id).gte('created_at', startOfDay)
    let recentSalesQuery = supabase.from('sales').select('*').eq('business_id', business.id).order('created_at', { ascending: false }).limit(5)
    ...
    const costMapResult = await getProductCostMap(supabase, business.id)
    ...
    const myList = items
      .filter((it: any) => saleUserMap[it.sale_id] === userId && it.quantity > 0)
